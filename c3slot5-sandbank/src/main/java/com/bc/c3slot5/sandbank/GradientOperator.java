...
public class GradientOperator {
    ...
    public double[][] computeGradient(double[] sourceData,
        ...
        int kernelRadius = SandbankOp.convolutionFilterKernelRadius;
        ...
        double[][] xData = xConvolution.makeConvolution(sourceData, sourceWidth, sourceHeight, flagArray);
        double[][] yData = yConvolution.makeConvolution(sourceData, sourceWidth, sourceHeight, flagArray);
        ...
        for (int y = 0; y < sourceHeight; y++) {
            for (int x = 0; x < sourceWidth; x++) {
                ...
                gradientMagnitudeArray[y * (sourceWidth) + x] =
                        Math.sqrt((xData[x][y] * xData[x][y]) + (yData[x][y] * yData[x][y]))/(weightingFactor);
        ...
        SandbankOp.makeFilledBand(gradientMagnitudeArray, sourceWidth, sourceHeight,
                targetTileGradientMagnitude, SandbankOp.maxKernelRadius);
        SandbankOp.makeFilledBand(gradientDirectionArray, sourceWidth, sourceHeight
                , targetTileGradientDirection, SandbankOp.maxKernelRadius);
