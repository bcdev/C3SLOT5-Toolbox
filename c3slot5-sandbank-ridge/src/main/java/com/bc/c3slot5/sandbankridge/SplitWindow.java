...
import com.bc.c3slot5.sandbankridge.CayulaFrontsTest;
...
public class SplitWindow {
    ...
    public void compute(double[] sourceData,
        ...
        int  histogramBins = SandbankRidgeOp.standardHistogramBins;
        int step = (int) (windowSize * SandbankRidgeOp.windowOverlap / 100.);
        ...
        for (int k = 0; k <= height_count; k = k + step) {
            for (int l = 0; l <= width_count; l = l + step) {
                ...
                if (histogramBins == 0) {
                    histogramBins = SandbankRidgeOp.standardHistogramBins;
                }
        ...
        SandbankRidgeOp.extractSubArray(sourceWidth, sourceHeight,targetWidth, frontsData, frontsCayulaArray);
    }
}